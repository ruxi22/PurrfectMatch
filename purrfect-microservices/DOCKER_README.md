...
2. **Import the Collection**:
   ...
   - Select the file: `Purrfect-Microservices.postman_collection.json`
   - The collection will appear in your Postman workspace

### Using the Collection

The collection includes organized requests for all services:

- **1. Auth Service** - User registration, login, user management
- **2. Pet Service** - Pet catalog operations (CRUD)
- **3. Adoption Service** - Adoption request workflow
- **4. Notification Service** - Notification management
- **5. Health Checks** - Service health monitoring
- **6. Integration Test Scenario** - Complete end-to-end workflow

### Important: Port Configuration

**Note**: The Postman collection uses port `8080` by default. Since the Docker setup uses port `8090` for the API Gateway, you have two options:

**Option 1: Update Postman Collection (Recommended)**
- After importing, use Postman's **Find and Replace** feature:
  - Find: `localhost:8080`
  - Replace: `localhost:8090`
- Or manually update each request URL

**Option 2: Use Environment Variables**
- Create a Postman environment with variable:
  - Variable: `base_url`
  - Value: `http://localhost:8090`
- Update collection URLs to use `{{base_url}}`
...
3. **Run requests in order**:
   - Start with **Auth Service** (Register a user)
   - Then **Login** to get JWT token
   - Use the token for authenticated requests
   - Test **Pet Service** endpoints
   - Test **Adoption Service** endpoints
   - Check **Notifications**

4. **Use Integration Test Scenario** folder for a complete end-to-end test
